...
from src.utils import str_to_dict
...
def main(args):
    ...
    model = src.create_model(args.model, pretrained=True)
    model.to(device)
    test_transform = src.create_transform(model)
    method_kwargs = args.method_kwargs or {}
    method = src.create_detector(args.method, model=model, **method_kwargs)
    hyperparameters = src.create_hyperparameters(args.method)
    best_kwargs = {}
    ...
    test_pipeline = src.create_pipeline(
